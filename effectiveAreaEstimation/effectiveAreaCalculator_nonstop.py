...
def main(input_pre, input_post, mc_input, model_path, output, eMin, eMax, zenMax, ebins, zenbins, n_batches,
    ...
    for i in range(len(input_pre_list)):
        ...
        for n, batches in enumerate(zip(intervals_pre, intervals_post)):
            ...
            post_data = post_data[zenith_splinempe < cut]

            post_data.reset_index(inplace=True, drop=True)
            post_data['Hoinka_Labels_zenith_true_cos'] = np.cos(post_data.Hoinka_Labels_zenith_true)
            ...
            post_data['zen_bin'] = pd.cut(post_data['Hoinka_Labels_zenith_true_cos'], zen_bins)
            post_data_agg = post_data[['MuonWeight', 'zen_bin']].groupby('zen_bin').sum()

            if result is None:
                result = pd.concat({'predicted_count': post_data_agg, 'total_count': pre_data_agg}, axis=1)
                result.fillna(0, inplace=True)
            else:
                result.predicted_count += post_data_agg.fillna(0)
    ...
    result.to_csv("%s/effArea_mgs_nonstop.csv" % output, sep='\t')

    joblib.dump(result, "%s/effArea_mgs_nonstop.pickle" % output)
    ...
    result_mc = result_mc[result_mc.zenith < cut]
    ...
    result_mc_agg_total.fillna(0.0, inplace=True)

    # store aggregation result
    result_mc_agg_total.to_csv("%s/effArea_mgs_corsica_total.csv" % output, sep='\t')

    joblib.dump(result_mc_agg_total, "%s/effArea_mgs_corsica_nonstop_total.pickle" % output)
