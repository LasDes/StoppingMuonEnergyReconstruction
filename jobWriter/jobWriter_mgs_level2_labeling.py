...
def main(config_file):
    ...
    check_path(job_path + "MGS_level2_labels_14")

    write_dagman_config(job_path + "MGS_level2_labels_14/dagman.config")

    write_submit_file(job_path + "MGS_level2_labels_14/submit.sub",
                      conf.get("exec", "ice_rec"),
                      conf.get("out", "logs"),
                      conf.get("exec", "mgs_labeling"),
                      "--sim")

    write_dag_fe(job_path + "MGS_level2_labels_14/job.dag",
                 	conf.get("src", "src_14"),
                 	conf.get("out", "out_14"),
                 	"submit.sub",
