...
def write_dag_fe(filename, source, output, submit_file, geometry, id,
                 every_other=1):
    files = glob.glob(source + "*/clsim-base-4.0.3.0.99_eff/*.bz2")
...
def main(config_file):
    ...
    check_path(job_path + "FE_11374")

    write_dagman_config(job_path + "FE_11374/dagman.config")

    write_submit_file(job_path + "FE_11374/submit.sub",
                      conf.get("exec", "ice_rec"),
                      conf.get("out", "logs"),
                      conf.get("exec", "feature_extraction"),
                      "--sim")

    write_dag_fe(job_path + "FE_11374/job.dag",
                 	conf.get("src", "src_11374"),
                 	conf.get("out", "out_11374"),
                 	"submit.sub",
                 	conf.get("data", "geometry"),
                 	11374, every_other=int(conf.get("src", "pck_11374")))
