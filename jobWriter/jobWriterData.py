...
def write_dag_fe(filename, source, output, submit_file, geometry, id, month,
                 every_other=1):
    files = glob.glob(source + "%s*/*/*.bz2" % month)
...
def main(config_file):
    ...
    check_path(job_path + "data")

    write_dagman_config(job_path + "data/dagman.config")

    write_submit_file(job_path + "data/submit.sub",
                      conf.get("exec", "ice_rec"),
                      conf.get("out", "logs"),
                      conf.get("exec", "feature_extraction"))

    write_dag_fe(job_path + "data/job.dag",
                 conf.get("src", "src"),
                 conf.get("out", "out"),
                 "submit.sub",
                 ...
                 11057,
                 conf.get("src", "month"),
                 every_other=int(conf.get("src", "picks")))
