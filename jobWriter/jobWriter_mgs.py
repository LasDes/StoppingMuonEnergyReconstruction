...
def main(config_file):
    ...
    check_path(job_path + "MGS_42")

    write_dagman_config(job_path + "MGS_42/dagman.config")

    write_submit_file(job_path + "MGS_42/submit.sub",
                      conf.get("exec", "ice_rec"),
                      conf.get("out", "logs"),
                      conf.get("exec", "feature_extraction"),
                      "--sim")

    write_dag_fe(job_path + "MGS_42/job.dag",
                 	conf.get("src", "src_42"),
                 	conf.get("out", "out_42"),
                 	"submit.sub",
