"""correlationStudy_nu.
Usage: correlationStudy_nu.py MC MODELS OUTPUT BATCHES
...
def load_and_prepare_data(input_files, batch, modifier=None):
    ...
    lab, att, wgt, grp = load_data(input_files, batch, verbosity=False, modifier=modifier)
...
def analyse_model(mc_input, model_input, output, n_batches):
    ...
    for f in mc_input.split(","):
        ...
        for n, batch in enumerate(intervals):
            ...
            lab, att, wgt, grp = load_and_prepare_data(file, batch, modifier='nu')
    ...
    joblib.dump(results, "%s/df_correlationData_nu.pickle" % output)
