...
def analyse_model(mc_input, model_input, threshold=0.9):
    ...
    print("Get Cut for m-classification")

    cuts = np.linspace(0.0, 1.0, 101)
    purities = np.zeros(len(cuts))
    print("Number of events: %.6f Hz" % np.sum(wgt))
    print("Number of single stopping events: %.6f Hz" % np.sum(wgt[lab_m == 1]))
    for i in range(len(cuts)):
        try:
            purities[i] = np.sum(wgt[(lab_m == 1) & (proba_m > cuts[i])]) / np.sum(wgt[proba_m > cuts[i]])
        except:
            purities[i] = 0.0
        if purities[i] > threshold:
            print("Cut: %.3f, Puritiy: %.5f, Frequency: %.6f" % (
                cuts[i], purities[i], np.sum(wgt[proba_m > cuts[i]])))
            break
